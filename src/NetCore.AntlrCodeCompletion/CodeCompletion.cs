...
namespace NetCore.AntlrCodeCompletion
{
    /// <summary>
    /// </summary>
    /// <remarks>
    ...
    public class CodeCompletion
    {
        ...
        public CodeCompletion(Parser parser, ISet<int> preferredRules, ISet<int> ignoredTokens)
        {
            this.parser = parser;
            this.atn = parser.Atn;
        ...
        private ISet<int> ProcessRule(ATNState startState, int tokenIndex, LinkedList<int> callStack, string indentation)
        {
            ...
            while (statePipeline.Count() > 0)
            {
                ...
                {
                    // Happens only for the first state in this rule, not sub rules.
